...
Vagrant.configure(2) do |config|
  ...
  # using a specific IP.
  ###
  # IMPRTANT WITH NFS: NFS requires a host-only network with a static IP to be created.
  ###
  ...
  # config.vm.synced_folder "./web/www", "/var/www/hostdata", nfs:true
  #
  ...
  config.vm.provision :shell, :path => "Vagrant/2017/provision.sh"
  config.vm.provision :shell, :path => "Vagrant/2017/www_php7_apache.sh"
  config.vm.provision :shell, :path => "Vagrant/2017/www_php7_mysql.sh"
